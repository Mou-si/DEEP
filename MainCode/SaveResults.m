...
try
    nccreate(FileFullName, 'Map', ...
        'Dimensions', {'x', size(PolynyaIDMap, 1), 'y', size(PolynyaIDMap, 2)}, ...
        'Datatype', 'int32', ...
        'FillValue', int32(-999), ...
        'DeflateLevel', 5);
    ncwriteatt(FileFullName, 'Map', ...
        'LongName', 'Map of daily edge of each polynya')
    ncwriteatt(FileFullName, 'Map', ...
        'Description', Descriptions{4})
end
try
    nccreate(FileFullName, 'IDs', ...
        'Datatype', 'int32', ...
        'Dimensions', {'IDs', size(CoastalPolynyasforDay, 1)});
    ncwriteatt(FileFullName, 'IDs', ...
...
ncwrite(FileFullName, 'Map', PolynyaIDMap);
ncwrite(FileFullName, 'IDs', CoastalPolynyasforDay(:, 1));
...
try
    ...
    elseif strcmp(NCCreateError.identifier, ...
            'MATLAB:imagesci:netcdf:variableExists')
        nccreate([In.Save.Path, '/LonLat.nc'], 'x', ...
        'Datatype', 'int16', ...
        'Dimensions', {'x', size(In.SICLon, 1)});
    else
        disp(['<strong>UNKNOW ERROR: </strong>', NCCreateError.message]);
