...
% T2m
t2m_120L_We = reshape(t2m_120L, 214, 20, size(t2m_120L, 1));
t2m_120L_We(:, 9 : 10, :) = [];
temp = t2m_120L_We;
...
t2m_120R_We = reshape(t2m_120R, 214, 20, size(t2m_120R, 1));
t2m_120R_We(:, 9 : 10, :) = [];
temp = t2m_120R_We;
temp = reshape(temp(3 : end - 2, :), 7, 30, size(temp, 2));
temp = permute(temp, [1, 3, 2]);
...
% wind speed
U_120L_We = reshape(U_120L, 214, 20, size(U_120L, 1));
U_120L_We(:, 9 : 10, :) = [];
temp = U_120L_We;
temp = reshape(temp(3 : end - 2, :), 7, 30, size(temp, 2));
temp = permute(temp, [1, 3, 2]);
...
U_120R_We = reshape(U_120R, 214, 20, size(U_120R, 1));
U_120R_We(:, 9 : 10, :) = [];
temp = U_120R_We;
temp = reshape(temp(3 : end - 2, :), 7, 30, size(temp, 2));
temp = permute(temp, [1, 3, 2]);
...
ErrorBarPlot(6 : 7 : 214, PolynyaArea120LWe(1, :), PolynyaArea120LWe(2, :) ./ 2, ...
    ...
    'error', 'FaceAlpha', 0.1)
ylabel('Infrequency (\times10^4 km^2)')
...
ErrorBarPlot(6 : 7 : 214, PolynyaArea120RWe(1, :), PolynyaArea120RWe(2, :) ./ 2, ...
    ...
    'error', 'FaceAlpha', 0.1)
ylabel('Frequency (\times10^5 km^2)')
...
ErrorBarPlot(6 : 7 : 214, PolynyaArea120LMWe(1, :), PolynyaArea120LMWe(2, :) ./ 2, ...
    ...
    'error', 'FaceAlpha', 0.1)
ylabel('Infrequency (\times10^3 km^2)')
set(gca, 'YColor', [0, 0, 0.5], 'XColor', 'k')
set(gca, 'XLim', [0, 215], ...
    'XTick', Mo(1, 1 : 2 : end), 'XTickLabel', {'Apr', 'Jun', 'Aug', 'Oct'}, ...
    'FontSize', 8)
...
ErrorBarPlot(6 : 7 : 214, PolynyaArea120RMWe(1, :), PolynyaArea120RMWe(2, :) ./ 2, ...
    ...
    'error', 'FaceAlpha', 0.1)
ylabel('Frequency (\times10^3 km^2)')
set(gca, 'YColor', [0.5, 0, 0], 'XColor', 'k')
set(gca, 'XLim', [0, 215], ...
    'XTick', Mo(1, 1 : 2 : end), 'XTickLabel', {'Apr', 'Jun', 'Aug', 'Oct'}, ...
    'FontSize', 8)
...
subplot(2, 2, 3)
yyaxis left
ErrorBarPlot(6 : 7 : 214, t2m_120L_We(1, :) - 273.15, t2m_120L_We(2, :) ./ 2, ...
    'line', 'LineStyle', '-', 'LineWidth', 1.5, 'Color', [0, 0, 0.7], ...
    'error', 'FaceAlpha', 0.1)
hold on
ErrorBarPlot(6 : 7 : 214, t2m_120R_We(1, :) - 273.15, t2m_120R_We(2, :) ./ 2, ...
    'line', 'LineStyle', '-', 'LineWidth', 1.5, 'Color', [0.7, 0, 0], ...
    'error', 'FaceAlpha', 0.1)
