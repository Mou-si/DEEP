...
In.TimeTotal = ... The time you want to identify polynyas
    ...
    datetime('2013-01-01') : datetime('2023-01-31')];
% In.TimeTotal = ... The time you want to identify polynyas
...
In.SICFile.Name2 = ...%     {'-v5.4.hdf'};
    {'-PSSM-AMSR-36.mat', '-PSSM-AMSR-36.mat'};
In.SICFile.VarName = ...
    'Polynya';
% read the Lon & Lat
% In.SICLon = hdfread(...
%     [In.SICFile.Dir, '\LongitudeLatitudeGrid-s25000-Antarctic.hdf'], 'Longitudes');
% In.SICLat = hdfread(...
%     [In.SICFile.Dir, '\LongitudeLatitudeGrid-s25000-Antarctic.hdf'], 'Latitudes');
In.SICLon = load('G:\Antaratica_ASI_SIC_25000\LonLat_25000.mat');
In.SICLat = In.SICLon.Lat;
In.SICLon = In.SICLon.Lon;
% read land mask
In.SICFile.LandMask = ncread([In.SICFile.Dir, '\LandMaskAMSR25000.nc'], 'LandMask');
% set the resolution
In.Resolution = 25;

%% Land Fast Ice
In.FastIceFlag = FastIceParameter_25km;
...
%% SaveResults
In.Save.Path = ...
    'G:\DEEP-AAShare\PSSM85_25km_14d';
In.Save.FileName1 = ...
    'DEEP_s25000_AMSR_PSSM_';
In.Save.FileName2 = ...
    '_v1.0.nc';
