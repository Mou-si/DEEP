...
SICFiles = cat(1, SICFiles(3 : end - 1).name);
...
for i = 1 : length(SICFiles)
    ...
    % read data
    try
    SIC60mapDaily = ncread([SIC60Path, SICFiles(i, :)], 'PolynyaIDMap');
    catch
        continue
    end
    ...
    % manual RONP
    temp = ismember(SIC60mapDaily, [43064760, 53001750, 63078770, 103011750, 122989750, 213005760]);
...
figure
m_proj('Azimuthal Equal-area', 'lon', 0, 'lat', -90, 'rad', 31, 'rec', 'on')
...
hold on
[~, h] = m_contour(Lon2, Lat2, OverviewMap, [0.5, 0.5]);
set(h, 'LineColor', 'r')
